# This script aims to explore changes in BODY COMP in middle age NZO after different stages of feeding:
...
#WE WANT TO EXPLORE IF THERE IS MICE THAT ARE OBESITY PRONE AND OBESITY RESISTANT
#WE WILL USE FAT MASS AS A VARIABLE TO DEFINE THIS
...
echoMRI_data <- read_csv("~/Documents/GitHub/data/data/echomri.csv") %>%
  ...
  mutate(
    ...
    STATUS = case_when(
      ...
      Date %in% as.Date(c("2025-04-28","2025-05-05","2025-05-06")) ~ "BW loss",
...
# Step 1. Get peak obesity data
peak_data <- echoMRI_data %>%
...
# Step 2. Classify into OP / OR (quartiles)
quantiles <- quantile(peak_data$Weight, probs = c(0.25, 0.75), na.rm = TRUE)

peak_data <- peak_data %>%
  mutate(OP_status = case_when(
    Weight >= quantiles[2] ~ "OP",  # top 25%
    Weight <= quantiles[1] ~ "OR",  # bottom 25%
    TRUE ~ "Intermediate"
  ))

# Step 3. Merge back into main dataset
echoMRI_data <- echoMRI_data %>%
  left_join(peak_data %>% select(ID, OP_status), by = "ID") %>% 
  filter(!(ID == 3726 & Date == as.Date("2025-04-28")))  #repeated
  

ggplot(
  echoMRI_data %>% filter(STATUS == "peak obesity" & OP_status != "Intermediate"),
  aes(x = OP_status, y = Weight, fill = OP_status)
) +
  geom_boxplot(alpha = 0.6, outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha = 0.6, size = 2) +
  scale_fill_manual(values = c("OR" = "#317EC2FF", "OP" = "#C03830FF")) +
  theme_minimal() +
  labs(
    x = "Phenotype group",
    y = "BW (g)"
...
echoMRI_data <- read_csv("~/Documents/GitHub/data/data/echomri.csv") %>%
  ...
  mutate(
    ...
    OBESITY_CLASS = case_when(
      ID %in% c(3707, 3708, 3710, 3714, 3726, 3729) ~ "obesity prone",
      ID %in% c(3706, 3717, 3720, 3722, 3725, 3727) ~ "obesity resistant"
    ))   %>%
  select(ID, Date, Fat, Lean, Weight, n_measurement, adiposity_index, GROUP, DRUG,OBESITY_CLASS) %>%
  mutate(
    day_rel = Date - first(Date),
    STATUS = case_when(
      n_measurement == 1 ~ "baseline",
  ...
  filter(!is.na(STATUS) & !is.na(OBESITY_CLASS)) %>% 
  filter(!(ID == 3726 & Date == as.Date("2025-04-28")))  #repeated
...
plot <- echoMRI_data  %>%
  ggplot(aes(x = STATUS, y = adiposity_index, color = DRUG, group = ID)) +
 ...
 # stat_summary(
  ...
  theme_minimal() +
  labs(y = "Adiposity index (fat/lean mass)", color = "Drug", fill = "Drug") +
  facet_wrap(~OBESITY_CLASS*GROUP) +
  format.plot+
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1)
  )
