#We aim to evaluate changes in locomotion after injections of RTIOXA 43 in females and males C57
...
sable_loc_data_lights <- sable_dwn %>% 
  ...
  ungroup() %>% 
  group_by(ID, complete_days,SEX,DRUG,lights) %>% 
  mutate(is_complete_day = if_else(min(zt_time)==0 & max(zt_time)==23, 1, 0)) %>% 
  filter(!complete_days %in% c(0, 3)) %>% 
  filter(!ID %in% c(1003,2001)) #ID 1003 DIED DURING EXPERIMENT AND ID 2005 WAS IN CAGE 5

sable_loc_data_minutes_lights <- sable_loc_data %>%
  arrange(ID, complete_days, hr) %>%       # make sure data is ordered
  group_by(ID, DRUG, complete_days,SEX,lights) %>%    # group per animal, drug, and day
  mutate(
    locomotion = value - lag(value),       # change in meters
    locomotion = if_else(locomotion < 0, 0, locomotion),  # remove negative jumps
    moving_min = if_else(locomotion > 0, 1, 0)             # 1 min per movement
...
# data analysis----
#effect of drug in time spent moving considering SEX and MOVER

model <- lmer(
  avg_moving_hr ~ DRUG * SEX * mover + (1 | ID),
  data = sable_loc_data_minutes_movers
)
summary(model)
anova(model)
emmeans(model, pairwise ~ DRUG | SEX * mover, adjust = "tukey")

#effect of drug in time spent moving collapsing SEX but considering MOVER
model2 <- lmer(
  avg_moving_hr ~ DRUG * mover + (1 | ID),
  data = sable_loc_data_minutes_movers
)

anova(model2)
emmeans(model2, pairwise ~ DRUG | mover, adjust = "tukey") #nothing happens, boring

#SPA after RTIOXA 43 injections ----

sable_ped_data <- sable_dwn %>% 
  filter(COHORT == 6) %>%   
  mutate(lights  = if_else(hr %in% c(20,21,22,23,0,1,2,3,4,5), "off", "on"),
         SEX = case_when(
           startsWith(as.character(ID), "1") ~ "M",
...
sable_ped_data_minutes <- sable_ped_data %>%
  arrange(ID, complete_days, hr) %>%       # make sure data is ordered
  group_by(ID, DRUG, complete_days,SEX) %>%    # group per animal, drug, and day
  mutate(
    ped = value - lag(value),       # change in meters
    ped = if_else(ped < 0, 0, ped),  # remove negative jumps
    ped_min = if_else(ped > 0, 1, 0)             # 1 min per movement
  ) %>%
  drop_na() %>% 
  summarise(
    total_ped_min = sum(ped_min),    # total minutes moved per day
    total_distance_ped = sum(ped),      # total meters per day
    .groups = "drop"
  ) %>%
  group_by(ID, DRUG,SEX) %>%
  summarise(
    avg_ped_hr = mean(total_ped_min/60),  # average across days
    avg_ped_distance = mean(total_distance_ped),
    .groups = "drop"
  )


combined_df <- sable_ped_data_minutes %>% #SPA
  left_join(sable_loc_data_minutes_movers, by = c("ID", "DRUG", "SEX")) %>% 
  group_by(ID) %>% 
  mutate(SPA_hr = avg_moving_hr - avg_ped_hr,
         SPA_meters = avg_distance - avg_ped_distance) %>% 
  select(ID,DRUG,SEX,SPA_hr,SPA_meters,mover) %>% 
  mutate(
    ID = factor(ID),
    DRUG = factor(DRUG, levels = c("vehicle", "RTIOXA_43_medchem", "RTIOXA_43_donated")),
    SEX = factor(SEX)
...
# SPA in min MOVERS ----
ggplot(combined_df, aes(x = DRUG, y =  SPA_hr , fill = DRUG)) +
  geom_line(aes(group = ID), color = "gray50", alpha = 0.5) +   # connect the same ID across drugs
  stat_summary(fun = mean, geom = "col", color = "black", width = 0.7, alpha = 0.8) +   # bars with mean ± SEM
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.3) +
  geom_point(aes(group = ID), alpha = 0.7, size = 2,   # individual points
