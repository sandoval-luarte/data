# This script aims to explore changes in body weight gained (%) in middle age NZO after different stages of feeding:
...
plot <- BW_data %>%
  ggplot(aes(x = STATUS, y = bw_rel, color = DRUG, group = ID)) +
  ...
  theme_minimal() +
  labs(y = "Body Weight (BW) gained (%)", color = "Drug", fill = "Drug") +
  facet_wrap(~GROUP) +
  format.plot+
  theme(
