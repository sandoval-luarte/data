# This script aims to explore changes in adiposity index mass in middle age NZO after different stages of feeding:
...
plot <- lean_plotdata %>%
  ...
  theme_minimal() +
  labs(y = "adiposisty index (fat/lean mass)", fill = "Group", color = "Group") +
...
# Fit model
model <- lmer(adiposity_index ~ STATUS * GROUP * DRUG + (1|ID), data = echoMRI_data)
summary(model)
