...
BW_data <- read_csv("../data/BW_compilate.csv") %>% 
  filter(!ID %in% c(3715,3723)) #3712 died due sickness but we still can use the data, 3715 in echoMRI died and 3723 has issues with the sable data
 ...
 filter(Date <= "2025-02-24") %>% #DAY 1 OF RESTRICTION 
  ...
  mutate(day_rel = n_measurement - first(n_measurement),
         ...
         GROUP = case_when(
           ...
           ID %in% c(3708, 3714, 3720, 3721, 3710, 3722, 3724, 3725, 3727, 3728, 3729) ~ "restricted")) 
...
sable_TEE_data <- sable_dwn %>% # Load the data
  ...
  filter(!ID %in% c(3715,3723), is_complete_day ==1, complete_days==2) %>% #3715 died and 3723 has issues with the sable
  ungroup() %>% 
  group_by(ID,SABLE) %>% 
...
sable_TEE_summary <- sable_TEE_data %>%
  select(ID,SABLE,tee) %>% 
  group_by(ID, SABLE) %>%
  slice(1) %>%
  pivot_wider(
...
# t test
t.test(sable_TEE_summary$`baseline`, sable_TEE_summary$`peak obesity`, paired = TRUE)
...
plot_5 <- sable_TEE_summary %>% 
  ...
  stat_summary(aes(group = 1), fun = mean, geom = "point",
               color = "black", size = 1.2) +                 # mean point, black
  labs(x = "Weeks", y = "24h TEE (kcal)") +
  theme_classic() +
  format.plot
plot_5
