...
sable_locomotion_data <- sable_dwn %>% 
  ...
  filter(loc_act >= 0) %>%
  summarise(total_pedmeters = sum(loc_act), .groups = "drop") %>%
  ...
  group_by(ID, SABLE,STRAIN,SEX,DIET_FORMULA) %>%
  summarise(total_pedmeters = mean(total_pedmeters), .groups = "drop") %>%
...
write_csv(sable_locomotion_data, "../data/sable_pedmeters_data.csv") # Save as CSV
