#we aim to evaluate changes in RMR (30 min with less activity)
...
mouse3706 <- sable_TEE_data %>%     
  filter(SABLE=="baseline") %>% 
  mutate(
    kcal_diff = value - lag(value),                      # difference in kcal/hr vs previous timepoint
    kcal_diff = replace_na(kcal_diff, 0)
  )
