...
echoMRI_data_43_assignation2 <- read_csv("~/Documents/GitHub/data/data/echomri.csv") %>%
  ...
  mutate(
    DRUG_ASSIGNATION2 = case_when(
      ID %in% c(8096, 8102, 8097, 8098, 8101, 8099) ~ "RTIOXA_43", #8099 eliminated for now
      ID %in% c(8095, 8100, 8103) ~ "vehicle"
    )
  ) %>% 
...
#check if I have outliers in delta adiposity index
...
# group assignation without ID 8099 -----
## Differentiated analysis for RTIOXA 43 obtained from Yanan and from medchem----
...
echoMRI_data_43_assignation <- read_csv("~/Documents/GitHub/data/data/echomri.csv") %>%
  ...
  mutate(
    DRUG_ASSIGNATION = case_when(
      ID %in% c(8096, 8102) ~ "RTIOXA_43_Z", #8099 eliminated for now
      ID %in% c(8097, 8098, 8101) ~ "RTIOXA_43_M",
      ID %in% c(8095, 8100, 8103) ~ "vehicle"
    )
  ) %>% 
...
leveneTest(adiposity_index ~ DRUG_ASSIGNATION, data = echoMRI_data_43_assignation) #p = 0.50 > 0.05, so variances can be considered equal
...
#data analysis
#1 does rtioxa-43 decreases bw after 5 days of injection? WITHOUT 8099
#to check if variances differ between groups,
echoMRI_data_43 <- echoMRI_data_43 %>%
  mutate(DRUG = factor(DRUG))

leveneTest(delta_adiposity_index ~ DRUG, data = echoMRI_data_43) #p = 0.62 > 0.05, so variances can be considered equal
leveneTest(delta_fat ~ DRUG, data = echoMRI_data_43) #p = 0.45 > 0.05, so variances can be considered equal
leveneTest(delta_lean ~ DRUG, data = echoMRI_data_43) #p = 0.2 > 0.05, so variances can be considered equal
leveneTest(delta_bw ~ DRUG, data = echoMRI_data_43) #p = 0.29 > 0.05, so variances can be considered equal
...
# group assignation -----
## Collapsed RTIOXAs 43 WITHOUT 8099----
...
echoMRI_data_43_assignation2 <- read_csv("~/Documents/GitHub/data/data/echomri.csv") %>%
  ...
  mutate(
    DRUG_ASSIGNATION2 = case_when(
      ID %in% c(8096, 8102, 8097, 8098, 8101) ~ "RTIOXA_43", #8099 eliminated for now
      ID %in% c(8095, 8100, 8103) ~ "vehicle"
    )
  ) %>% 
  mutate(DRUG_ASSIGNATION2 = factor(DRUG_ASSIGNATION2, levels = c("vehicle", "RTIOXA_43"))) %>% 
...
leveneTest(delta_adiposity_index ~ DRUG, data = echoMRI_data_432) #p = 0.28 > 0.05, so variances can be considered equal
leveneTest(delta_fat ~ DRUG, data = echoMRI_data_432) #p = 0.43 > 0.05, so variances can be considered equal
leveneTest(delta_lean ~ DRUG, data = echoMRI_data_432) #p = 0.14 > 0.05, so variances can be considered equal
leveneTest(delta_bw ~ DRUG, data = echoMRI_data_432) #p = 0.17 < 0.05, so variances are not considered equal

summary(aov(delta_bw ~ DRUG, data = echoMRI_data_432)) #p = 0.89
summary(aov(delta_fat ~ DRUG, data = echoMRI_data_432)) #p = 0.06
summary(aov(delta_lean ~ DRUG, data = echoMRI_data_432)) #p = 0.75
summary(aov(delta_adiposity_index ~ DRUG, data = echoMRI_data_432)) #p = 0.08

#conclusion> There is no indication that RTIOXA-43 (collapsed) decreases body weight, fat mass, 
# or adiposity after 5 days without ID 8099
#Probably because data are underpowered (n=3 per vehicle group and n=5 per RTIOXA 43 group).
