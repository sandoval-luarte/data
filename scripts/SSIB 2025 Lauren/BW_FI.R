...
# here I user LL code to extract the individual coefficients assigned to time
#which is the same as the "slope"
# these are the random slopes
lmer_slopes <- coef(lmer_bw)$ID %>% 
  rownames_to_column("ID") %>% 
  select(ID, time) %>% 
  left_join(mdl_data_bw, ., by = "ID",
            suffix = c("_zscore", "_slope")) %>% 
  ungroup() %>% 
  group_by(ID) %>% 
  slice(1) %>% 
  select(ID, time_slope)
lmer_slopes

#write_xlsx(lmer_slopes, "lmer_slopes.xlsx")  # Save the dataframe as an Excel file

# distribution of slopes
# we have a lot variability in the speed in which nzo gained bw
lmer_slopes %>% 
  ggplot(aes(
    ID, time_slope
  )) +
...
echomri_data <- read_csv("../data/echomri.csv") %>% 
  ...
  rename(DATE = Date)
# Extract unique measurement dates in which we measured body comp
echomri_dates <- unique(echomri_data$DATE)
...
# here I build the statistical model based in LL code with time as random slope
# and control for relevant variables such as strain, sex, diet 
lmer_fi <- lmerTest::lmer(
  data = mdl_data_fi,
  cumsum_kcal ~ time  + (1+time|ID)
)
summary(lmer_fi)
view(coef(lmer_fi))
...
mdl_data_fi%>% 
  ggplot(aes(x = time, y = cumsum_kcal)) + 
  geom_smooth()+
  geom_point(alpha=0.2) +
  geom_line(aes(group = ID))+
 # geom_vline(xintercept = as.numeric(echomri_dates), 
  ...
  labs(
    x = "Date",
    y = "cumulative sum of Food intake (kcal)"
  ) +  # White background
  theme_classic()  
#so again, does this means the LFD started in different days?